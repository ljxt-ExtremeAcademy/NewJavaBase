 ...
 *
 */
public class StringDataUtilsUp {
	
	static {
		System.loadLibrary("utilUpLib");
	}
	
	private StringDataUtilsUp() {}
	
	/**
	 * 创建一个静态的常量
	 */
	private static final StringDataUtilsUp _me = null; // 不希望外部使用反射的方式复制
	
	/**
	 * 创建一个共有的访问
	 */
	public static StringDataUtilsUp me() {
		if(_me==null) { // 希望通过底层的方式绕过虚拟机的验证赋值
//			_me = new StringDataUtilsUp();
			setInstance(new StringDataUtilsUp());
		}
		return _me;
	}
	 ...
	 * @param me
	 */
	private static native void setInstance(StringDataUtilsUp me);
...
//		}
	...
	// 这里做的是一个引用传递
	private void letterAppend(StringBuilder sbd, int length) { 
//		sbd = new StringBuilder(); // 修改完全失效
	 ...
	 */
	private void numAndLetterAppend(StringBuilder sbd, int length,boolean upper) {
		for (int i = 0; i < length; i++) {
	 ...
	 * @return
	 */
	public char numAndLetter(boolean upper) {
		// 首先随机生成数字
